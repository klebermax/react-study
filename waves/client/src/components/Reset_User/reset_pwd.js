...
class ResetPwd extends Component {
  ...
  submitForm = event => {
    ...
    if (formIsValid) {
      axios
        .post(`${USER_SERVER}/reset_password`, {
          ...dataToSubmit,
          resetToken: this.state.resetToken
        })
        .then(response => {
          if (!response.data.success) {
            this.setState({ formError: true, formErrorMessage: response.data.message });
          } else {
    ...
    return (
      <div className="container">
        <form onSubmit={event => this.submitForm(event)}>
          ...
          <div>
            {this.state.formError ? <div className="error_label">{this.state.formErrorMessage}</div> : null}

            <button onClick={event => this.submitForm(event)}>Reset</button>
          </div>
        </form>
